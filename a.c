...
static void perf_arm(const char *cmd) {
    ...
    {char sk[64];snprintf(sk,64,"|%s|",cmd);if(strstr("|push|pull|sync|u|update|login|ssh|gdrive|email|install|send|j|job|pr|hub|create|repo|e|revert|cc|diff|d|perf|scan|review|fork|kill|ls|i|deps|log|serve|bench|",sk))return;}
    perf_arm_for(cmd);
}
