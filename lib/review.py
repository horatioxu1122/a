...
def run():
    items=json.loads(sp.run('gh search prs --state open --involves @me --json repository,number,title,url,author',shell=True,capture_output=True,text=True).stdout or '[]')
