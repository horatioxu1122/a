"""aio scan - Scan/clone repos. Usage: aio scan [loc] [name|#|#-#|all]"""
import sys, os, subprocess as sp, json
from pathlib import Path
from _common import init_db, load_proj, add_proj, DATA_DIR

def _clone(idxs, repos, pd):
    os.makedirs(pd, exist_ok=True)
    for i in idxs: n, u = repos[i][0], repos[i][1]; dest = f"{pd}/{n}"; r = sp.run(['gh', 'repo', 'clone', u, dest], capture_output=True, text=True); ok, _ = add_proj(dest) if r.returncode == 0 or os.path.isdir(dest) else (False, ''); print(f"{'✓' if ok else 'x'} {n}")
    if len(idxs)==1: open(f"{DATA_DIR}/cd_target",'w').write(f"{pd}/{repos[idxs[0]][0]}")

def _pick(repos, sel, fmt):
    pg=0
    while True:
        for i,r in enumerate(repos[pg*10:(pg+1)*10],pg*10): print(fmt(i,r))
        more=pg*10+10<len(repos); print(f"  [{len(repos)}] m=more" if more else "")
        if not sel: sel=input("\n#/all/q/search: ").strip()if sys.stdin.isatty()else None
        if sel=='m'and more: pg+=1; sel=None; continue
        if sel and sel[0]>'9'and sel not in('all','q'): repos=[r for r in repos if sel.lower()in(r[0]if isinstance(r,tuple)else r.name).lower()]; pg=0; sel=None; continue
        break
    if not sel or sel=='q': return []
    return list(range(len(repos)))if sel=='all'else[j for x in sel.replace(',',' ').split()for j in(range(int(x.split('-')[0]),int(x.split('-')[1])+1)if'-'in x else[int(x)])if 0<=j<len(repos)]

def run():
    init_db(); args = sys.argv[2:]; loc = 'loc'in args; args = [a for a in args if a not in('gh','loc')]
    sel = next((a for a in args if a.isdigit() or a == 'all' or (a.replace('-','').isdigit() and '-' in a)), None)
    name = next((a for a in args if a not in (sel,) and not a.startswith('-') and not a.startswith('/') and not a.startswith('~')), None)

    if not loc:
        r = sp.run(['gh', 'repo', 'list', '-L', '100', '--json', 'name,url,pushedAt'], capture_output=True, text=True)
        repos = sorted(json.loads(r.stdout or '[]'), key=lambda x: x.get('pushedAt',''), reverse=True)
        cloned = {os.path.basename(p) for p, _ in load_proj()}
        if name:
            match = next((r for r in repos if r['name']==name), None)
            if match and name not in cloned: _clone([0],[(name,match['url'],'')],os.path.expanduser('~')); return
            elif name in cloned: print(f"○ {name} already added"); return
        repos = [(r['name'],r['url'],r.get('pushedAt','')[:10]) for r in repos if r['name'] not in cloned and (not name or name.lower() in r['name'].lower())]
        if not repos: print("No new GitHub repos"); return
        idxs = _pick(repos, sel, lambda i,r: f"  {i}. {r[0]:<25} {r[2]}")
        if idxs: _clone(idxs, repos, os.path.expanduser('~'))
    else:
        d = os.path.expanduser(next((a for a in args if a not in (sel,)), '~'))
        existing = {p for p, _ in load_proj()}
        repos = sorted([p.parent for p in Path(d).rglob('.git') if p.exists() and str(p.parent) not in existing and '/.' not in str(p.parent)], key=lambda x: x.name.lower())[:50]
        if not repos: print(f"No new repos in {d}"); return
        idxs = _pick(repos, sel, lambda i,r: f"  {i}. {r.name:<25} {str(r)}")
        for i in idxs: ok, _ = add_proj(str(repos[i])); print(f"{'✓' if ok else 'x'} {repos[i].name}")
        if len(idxs)==1: open(f"{DATA_DIR}/cd_target",'w').write(str(repos[idxs[0]]))

run()
