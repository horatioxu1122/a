...
static void tm_ensure_conf(void) {
    ...
    fputs("# aio-managed-config\n"
        ...
        "set -g status 3\n"
        "set -g status-right \"\"\n"
        "set -g status-format[0] \"#[align=left][#S]#[align=centre]#{W:#[range=window|#{window_index}]#{?window_active,#[fg=colour232 bg=colour231 bold] ,#[fg=colour231 bg=colour243] } #I:#W #{?window_active, , }#[default]#[norange]}\"\n"
