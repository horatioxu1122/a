...
/* job cmd */
static void jcmd_fill(char*b,int cont,const char*wd){char fp[128]="";{const char*fk=strstr(wd,"/adata/forks/");if(fk)snprintf(fp,128,"a fork run '%s' ",fk+13);}char ctxf[P];snprintf(ctxf,P,"%s/a_ctx_%d.txt",TMP,(int)getpid());snprintf(b,B,"tmux splitw -vd -p50 -t $TMUX_PANE;a cat 1 >%s 2>/dev/null;while :;do %sclaude --dangerously-skip-permissions --append-system-prompt-file %s%s;e=$?;[ $e -eq 0 ]&&break;echo \"$(date) $e $(pwd)\">>%s/crashes.log;echo \"! crash $e, restarting..\";sleep 2;done;git add -A&&git diff --cached --quiet 2>/dev/null||{ m=$(head -1 .a_done 2>/dev/null||echo done);git commit -m \"job: $m\"&&git push -u origin HEAD 2>/dev/null&&gh pr create --fill 2>/dev/null&&a email \"[a job] $(basename $PWD)\" \"$m\";}",ctxf,fp,ctxf,cont?" --continue":"",LOGDIR);}
