...
omni.onsubmit=function(e){e.preventDefault();var t=performance.now(),it=qo.querySelectorAll('[data-c]'),raw=qi.value.trim(),v=views['/'+raw]?raw:(raw.indexOf(' ')>=0||!it[_cs])?raw:it[_cs].dataset.c;if(!v)return;qo.innerHTML='';if(views['/'+v]){go('/'+v);qi.value='';console.log('omni total '+(performance.now()-t).toFixed(2)+'ms');return}var t2=performance.now();fetch('/api/omni',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'q='+encodeURIComponent(v)}).then(function(r){return r.text()}).then(function(h){qo.innerHTML=h;qi.value='';qi.focus();console.log('omni fetch '+(performance.now()-t2).toFixed(2)+'ms')})};
