...
static void sync_repo(void) {
    ensure_git_id();
    char c[B];
    snprintf(c,B,"flock /tmp/.a_git.lock sh -c \"D='%s';rm -rf \\$D/.git/rebase-merge;[ -s \\$D/.git/index ]||git -C \\$D read-tree HEAD;git -C \\$D add -A;git -C \\$D commit -qm sync;git -C \\$D pull --rebase -q origin main&&git -C \\$D push -q origin main||(git -C \\$D fetch -q origin;git -C \\$D reset --hard origin/main) 2>/dev/null\"",SROOT);
    (void)!system(c);
}
